...
def train(inputs, targets, model, optimizer, n_epochs):
    ...
    for epoch in range(n_epochs):
        for i in range(batch_x.steps):
            ...
            for j in range(y.shape[1]-1):
                logits = model(x, y[:,j].view(-1,1))
                loss += model.loss_layer(logits, y[:,j+1])
            loss /= y.shape[1]
...
def main():
    ...
    model = Transformer(n_src_vocab=len(vocab), n_tgt_vocab=len(vocab),
                ...
                N=6, n_head=4, d_q=16, d_k=16, d_v=16, d_model=32, d_inner=128)
    model.cuda()
    ...
    train(inputs[:idx], targets[:idx], model, optimizer, n_epochs=100)
...
def visualize(x_ids, y_ids, pred_ids, vocab):
    ...
    for i in range(x_ids.shape[0]):
        x = [id2w[idx] for idx in x_ids[i] if idx != vocab['<pad>']]
        x = "".join(x)
...
def eval(model, vocab, inputs, targets, out_len=12):
    ...
    for i in range(batch_x.steps):
        ...
        for j in range(1, out_len):
            ...
            last_word = torch.argmax(logits[:,-1,:], dim=-1).view(-1, 1)
